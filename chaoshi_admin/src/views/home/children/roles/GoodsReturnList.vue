<template>
  <div>
    <!-- 导航 -->
    <mbx-nav one-nav="销售管理" two-nav="商品退货"></mbx-nav>
    <!-- 卡片试图 -->
    <el-card>
      <!-- 操作 区域 -->
      <el-row :gutter="30">
      ...
      <el-table :data="userList" stripe border class="user-table">
        ...
        <el-table-column label="操作" width="176">
          <template v-slot="scope">
            <el-button size="mini" type="primary" icon="el-icon-edit" @click="changeUserInfo(scope.row)">退货</el-button>
    ...
    changeClick(){
      ...
      this.$refs.changeForm.validate((reg) => {
        ...
        if(reg){
           ...
           * 修改请求
           */
          upNum(this.changeForm).then((res) => {
            // console.log(res)
            if(res.data.code === 0){
              this.changeUserShow = false
